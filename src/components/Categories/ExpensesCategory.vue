<template>
  <!-- <q-card class="my-card"> -->
    ...
    <q-card-section>
      <UIUniwersalItem
        v-for="category in accountStore.expenses"
        :key="category.id"
    ...
    />
    <q-card-section v-if="accountStore.expensesArchive.length">
      <q-expansion-item
        v-model="isExpansion"
        icon="sym_o_archive"
        label="Архів"
      ...
      >
        <UIUniwersalItem
          v-for="category in accountStore.expensesArchive"
          :key="category.id"
          :fromAccountColor="category.color"
          :fromAccountIcon="category.icon"
          :fromName="category.name"
...
<script setup>
  ...
  const confirmCreate = async (emit) => {
    if (emit.id) {
      ...
      await accountStore.categoryCreate(5, emit)
    }
