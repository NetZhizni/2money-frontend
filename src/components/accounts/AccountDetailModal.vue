...
const rangeBounds = computed(() => {
  ...
  if (range.value === 'all') {
    const earliestTx = accountTransactions.value.reduce(
      (min, t) => Math.min(min, t.date),
      props.account.createdAt,
    )
    const from = startOfDay(Math.min(earliestTx, props.account.createdAt))
    return { from, to }
  }
...
<template>
  <Modal title="Рахунок" @close="emit('close')">
    <div class="head">
      <IconCircle :icon="account.icon" :color="account.color" :size="64" />
      <div class="head-text">
