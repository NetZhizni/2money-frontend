...
/**
 * Wipes every account, transaction, recurring template, and receipt ("чек")
 * owned by the current profile. Categories and settings (base currency,
 * theme) are left untouched — they're configuration, not financial data, and
 * demo data never needed them cleared either (see stores/categories.ts's own
 * `removeAll` for that, a separate opt-in action). Only rows this profile
 * actually owns are removed (not cross-profile transfers someone else sent
 * us) — this is a reset of *my* data, not theirs.
 */
