<script setup lang="ts">
  ...
  const directTotals = computed(() => {
    ...
    for (const t of periodTransactions.value) {
      ...
      const categoryCurrency = resolveCategoryCurrency(
  ...
  function budgetLabel(categoryId: string): string | undefined {
    ...
    return t('categories.budgetLabel', {
      spent: formatMoney(p.spent, currency, opts),
      amount: formatMoney(p.amount, currency, opts),
    })
  }
  ...
  const ringSegments = computed(() => {
    const segs = categories
      ...
      .map((c) => ({
        ...
        name: c.name,
        color: c.color,
        amount: baseRolledTotals.value[c.id] ?? 0,
      }))
...
<template>
  ...
  <Teleport to="body">
    <button
      v-if="!readOnly"
      class="fab"
      :aria-label="t('categories.addCategoryAria')"
      @click="openCreate"
    >
      <MdiIcon
        name="mdiPlus"
