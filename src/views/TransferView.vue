<template>
  <q-card class="my-card">
    <q-card-section>
      <div class="text-h6">Перекази</div>
    </q-card-section>
    <q-card-section>
      <q-btn
        label="Створити пеереказ"
        @click="isOpenPopup = !isOpenPopup"
      />
    </q-card-section>
    <UIListTransactions
      v-for="(transactions, monthYear) in transactionStore.transactionTransferGrouped"
      :key="monthYear"
      ...
      <div class="my__card">
        <div class="card__bull">
          <Bill
            ...
            typeId="1"
            :options="
              accountStore.activeBalance.filter(
                (account) => account.id !== orderStore.order?.to_account_id?.id,
              )
            "
          />
          <Bill
            ...
            :options="
              accountStore.activeBalance.filter(
                (account) => account.id !== orderStore.order?.from_account_id?.id,
              )
            "
            :secondOptions="accountStore.notMyWallets"
          />
        </div>
        <AmountEditor
        ...
        <q-btn
          ...
          outline
          color="negative"
          @click="deleteOrder(orderStore.order?.id)"
        >
          Видалити
        </q-btn>
      </div>
...
<script setup>
  ...
  const deleteOrder = async (id) => {
    await transactionStore.transactionDelete(id)
  ...
  onMounted(async () => {
    await Promise.allSettled([accountStore.accountGetMy(), transactionStore.transactionGetMy()])
  })
</script>
